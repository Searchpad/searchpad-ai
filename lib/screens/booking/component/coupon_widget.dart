...
class _EnterCouponState extends State<EnterCoupon> {
  ...
  appCoupon() async {
    var resp = await applyCouponForBooking({
      "coupon_code": couponController.text.trim(),
      "customer_id": appStore.userId.toString()
    });
    if (resp['message']['status'] == true) {
      validCoupon = true;
      responseMessage = resp['message']['message'];
      couponData = CouponData(
        code: resp['message']['data']['code'],
        discount: resp['message']['data']['discount'],
        discountType: resp['message']['data']['discount_type'],
        id: resp['message']['data']['id'],
        expireDate: resp['message']['data']['expire_date'],
        status: resp['message']['data']['status'],
      );
      ...
      children: [
        ...
        Form(
          ...
          child: AppTextField(
            ...
            decoration:
                inputDecoration(context, labelText: language.couponCode),
          ).paddingAll(10),
        ),
        if (validCoupon != null)
          Text(
            responseMessage,
            style: primaryTextStyle(
                color: validCoupon == true ? greenColor : redColor,
