...
Future createUser(Map request) async {
  var resp = await (handleResponse(await buildHttpResponse('register',
...
Future<void> saveUserData(UserData data) async {
  if (data.apiToken.validate().isNotEmpty) appStore.setToken(data.apiToken!);
...
Future<void> clearPreferences() async {
  ...
  await appStore.setPrivacyPolicy('https://munasbat.thtbh.com/privacy-policy');
  await appStore.setTermConditions('https://munasbat.thtbh.com/term-condition');
  await appStore.setInquiryEmail('https://munasbat.thtbh.com/help-support');
...
Future<ServiceResponse> searchServiceAPI({
  String categoryId = '',
  String providerId = '',
  String isPriceMin = '',
  String isPriceMax = '',
  ...
  var res;
  try {
    res = ServiceResponse.fromJson(await handleResponse(
      await buildHttpResponse(
          'search-list?$categoryIds$customerId$providerIds$isPriceMinPara$isPriceMaxPara$subCategorys$searchPara$latitudes$longitudes$isFeatures$pages$perPages'),
    ));
    ...
    list.addAll(res.serviceList);
    product.addAll(res.product);

    lastPageCallBack?.call(res.serviceList.length != PER_PAGE_ITEM &&
        res.product.length != PER_PAGE_ITEM);
...
//region Provider Api
Future<prov.ProviderInfoResponse> getProviderDetail(int id,
    {int? userId}) async {
  try {
    prov.ProviderInfoResponse res = prov.ProviderInfoResponse.fromJson(
...
Future<BaseResponseModel> addWishList(request) async {
  Map<dynamic, dynamic> newRequest = {
    "id": "",
  ...
  return BaseResponseModel.fromJson(await handleResponse(
      await buildHttpResponse('save-favourite',
          method: HttpMethodType.POST, request: newRequest)));
}
...
// Fetch Admin Chat
Future<AdminChatModel> getChatList(
    int limit, int offset, conversationId) async {
  try {
    var response = await handleResponse(await buildHttpResponse(
        'get-chat-messages/$conversationId?limit=$limit&offset=$offset.',
        method: HttpMethodType.GET));
    AdminChatModel? resp;

    if (response['status'] == true) {
      return resp = AdminChatModel.fromJson(response);
    } else {
      resp = AdminChatModel(
          data: [], limit: null, offset: null, status: false, totalSize: null);
    }
    appStore.setLoading(false);
    return resp;
  } catch (e) {
...
// Send Admin Message
Future<dynamic> sendAdminMessage(Map data) async {
  try {
    dynamic res = await handleResponse(await buildHttpResponse('send-message',
        request: data, method: HttpMethodType.POST));
...
// Send Admin Message
Future<AdminChatModel> getLatestAdminMessage(conversationId, lastChatId) async {
  try {
    AdminChatModel res = AdminChatModel.fromJson(await handleResponse(
        await buildHttpResponse('get-new-messages/$conversationId/$lastChatId',
            method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
...
Future<ConversationModel> getConversations(limit, offset) async {
  try {
    ConversationModel res = ConversationModel.fromJson(await handleResponse(
        await buildHttpResponse('get-chat-conversastion?limit=10&offset=1.',
            method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
...
/// Fetch Providers
Future<ProvidersModel> getProvidersData(limit, offset, search, userId) async {
  try {
    ProvidersModel res = ProvidersModel.fromJson(await handleResponse(
        await buildHttpResponse(
            'providers-list?per_page=$limit&page=$offset&search=$search&customer_id=userId',
            method: HttpMethodType.GET)));
...
Future<ProductListModel> getProductList({
  ...
  try {
    ...
    final baseUrl = 'product-list';

    // Create a map to hold query parameters
    final queryParams = {
      'per_page': dataCount.toString(),
    ...
    final fullUrl = Uri(path: baseUrl, queryParameters: queryParams).toString();

    // Make the API request
    ProductListModel res = ProductListModel.fromJson(await handleResponse(
        await buildHttpResponse(fullUrl, method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
Future<ProductDetailModel> getProductDetail(prodId, userId) async {
  try {
    ProductDetailModel res = ProductDetailModel.fromJson(await handleResponse(
        await buildHttpResponse('product-details/$prodId?customer_id=$userId',
            method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
...
///  Product add to cart
Future productAddToCart(Map data) async {
  try {
    // await deleteAllCartList();
    var res = await handleResponse(await buildHttpResponse('carts/add',
        method: HttpMethodType.POST, request: data));
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
/// Fetch Cart List
Future<CartListModel> getAllCartList() async {
  try {
    CartListModel res = CartListModel.fromJson(await handleResponse(
        await buildHttpResponse('carts/list', method: HttpMethodType.GET)));
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
Future deleteAllCartList() async {
  try {
    var res = await handleResponse(
        await buildHttpResponse('carts/delete', method: HttpMethodType.POST));
...
/// Remove Single Cart
Future removeSingleCart({required Map data}) async {
  try {
    var res = await handleResponse(await buildHttpResponse('carts/remove',
        method: HttpMethodType.POST, request: data));
...
Future getCartSummary() async {
  try {
    var res = await handleResponse(
        await buildHttpResponse('carts/summary', method: HttpMethodType.GET));
    appStore.setLoading(false);
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
/// Create address
Future addShippingAddres(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
        'carts/update-shipping-address',
        method: HttpMethodType.POST,
        request: data));
...
/// Apply coupon
Future applyCoupon(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse('coupon-apply',
        method: HttpMethodType.POST, request: data));
...
/// Get Paymetn Options
Future<PaymentOptionModel> getPaymentListOption() async {
  try {
    PaymentOptionModel res = PaymentOptionModel.fromJson(
        await handleResponse(await buildHttpResponse(
      'payment-types',
      method: HttpMethodType.GET,
    )));
...
Future placeOrder(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse('order-place',
        method: HttpMethodType.POST, request: data));
    return res;
  } catch (e) {
    appStore.setLoading(false);
...
Future<OrderListModel> fetchOrderList(
    {dataCount, offset, required orderStatus}) async {
  try {
    OrderListModel res =
        OrderListModel.fromJson(await handleResponse(await buildHttpResponse(
      'order-list?per_page=$dataCount&page=$offset&payment_status=&order_status=$orderStatus',
      method: HttpMethodType.GET,
    )));
    appStore.setLoading(false);
    return res;
...
// Fetch Order Detail
Future<OrderDetailModel> ferOrderDetails({required orderId}) async {
  try {
    OrderDetailModel res =
        OrderDetailModel.fromJson(await handleResponse(await buildHttpResponse(
...
// Fetch Order Item Detail
Future<OrderItemDetailModel> ferOrderItemDetails({required orderId}) async {
  try {
    OrderItemDetailModel res = OrderItemDetailModel.fromJson(
        await handleResponse(await buildHttpResponse(
      'order-items/$orderId',
      method: HttpMethodType.GET,
    )));
...
// Fetch category base providers
Future<CategoryWiseProviderModel> fetchCategoryBasedSellers(
    {required catId, required customerId}) async {
  try {
    CategoryWiseProviderModel res = CategoryWiseProviderModel.fromJson(
        await handleResponse(await buildHttpResponse(
      'category-based-providers-list?id=$catId&customer_id=$customerId',
      method: HttpMethodType.GET,
    )));
    appStore.setLoading(false);
    return res;
...
// Create Address
Future createShippingAddress(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
...
// Edit Address
Future editShippingAddress(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
...
// Delete Address
Future deleteShippingAddress(addressId) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
...
//  Address List
Future<AddressListModel> fetchAddressList({dataLimit, offset}) async {
  try {
    var res =
        AddressListModel.fromJson(await handleResponse(await buildHttpResponse(
      'get-shipping-address?per_page=$dataLimit&page=$offset',
      method: HttpMethodType.GET,
    )));
    return res;
  } catch (e) {
...
// Apply coupon for bookings
Future applyCouponForBooking(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
...
Future checkPaymentStatus(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
...
// check payment status
Future checkServicePaymentStatus(Map data) async {
  try {
    var res = await handleResponse(await buildHttpResponse(
